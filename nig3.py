...
class LSTMDecoder(nn.Module):
    ...
    def __init__(self, vocab_size, embed_dim, hidden_dim, num_layers=4, dropout=0.1):
        ...
        # Removed attention; decoder operates purely on embeddings and its hidden state
        self.lstm = nn.LSTM(
            embed_dim, hidden_dim, num_layers,
    ...
    def forward(self, x, hidden_state, encoder_outputs, encoder_mask=None):
        ...
        # LSTM forward without attention context
        output, hidden_state = self.lstm(embedded, hidden_state)
        
        # Final output projection from decoder hidden state only
        predictions = self.output_projection(output)
        
        # Return None for attention weights to keep call signature compatible
        return predictions, hidden_state, None
